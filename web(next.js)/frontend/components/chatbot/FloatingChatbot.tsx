  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...
        {
          role: "assistant",
          content: "Berdasarkan rilis resmi BPS Provinsi Sulawesi Tengah terbaru, indikator terkait " + userPrompt + " menunjukkan pertumbuhan ekonomi 11.91%, IPM 71.38, dan kemiskinan turun ke 11.77%.",
  ...
  return (
    <>
      <div className="fixed bottom-7 right-7 z-50 flex items-center gap-3">
        {!isOpen && (
          <div className="hidden sm:block rounded-xl border border-orange-300 bg-slate-900 px-4 py-2 text-xs font-extrabold text-white shadow-xl">
            💬 Tanya STATIX BPS AI
          </div>
        )}
        <button
          onClick={() => setIsOpen(!isOpen)}
          className="flex h-16 w-16 items-center justify-center rounded-full border-2 border-white bg-gradient-to-tr from-[#F58220] to-[#EA580C] text-white shadow-2xl transition-all duration-300 hover:scale-110 hover:shadow-orange-500/50"
      ...
      {isOpen && (
        <div className="fixed bottom-28 right-7 z-50 flex h-[520px] w-[90vw] max-w-[420px] flex-col overflow-hidden rounded-3xl border-2 border-orange-300 bg-white shadow-2xl backdrop-blur-2xl">
          ...
          <div className="flex-1 space-y-3 overflow-y-auto p-4">
            {messages.map((m, idx) => {
              ...
              const bubbleClass = isUser ? "bg-slate-900 text-white" : "border border-orange-200/70 bg-orange-50/60 text-slate-800";
              return (
                <div key={idx} className={"flex gap-2.5 " + alignClass}>
                  ...
                  <div className={"max-w-[80%] rounded-2xl p-3 text-xs leading-relaxed " + bubbleClass}>
                    <div>{m.content}</div>
